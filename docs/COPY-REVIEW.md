...
**A NEW ITEM TAKES THEIR PLACE, and it is smaller but visible.** Two account executives were added the same day from supplied photographs, and **no names came with them** - a job title and two pictures, nothing else. Rather than guess, both entries carry the visible placeholder **"Name to come"** under an `ACCOUNT EXECUTIVE` label, and their asset slugs are positional (`account-executive-01`, `-02`). That is live on the page. **Send the two names and it is a four-line change**; the header of `content/team.ts` has the steps. Their portraits are generative on the same terms as Arsenio's - see the photography item below.
