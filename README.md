...
**The browser-tab icon is generated from the nav's own mark.** `app/icon.svg` carries the `MARK` path out of `components/ui/logo.tsx` verbatim, and `node scripts/make-favicon.mjs` cuts `app/favicon.ico` (16/32/48, packed by hand because sharp cannot write an ICO) and `app/apple-icon.png` (180). It is white on a `#151515` tile rather than `currentColor`: a favicon has no band to follow and has to read on a light browser chrome as well as a dark one. The reason it is scripted at all is that the old icon still carried the mark's blue gradient two days after the lockup went flat, so the tab and the nav were showing two different logos.
...
**Bruno was then replaced twice more.** His third needed no re-tuning - square-on, head centroid at 0.491, so `faceX: 0.5` carried straight over. His fourth is the widest master in the set: standing, holding a notebook, looking off-camera. `zoom: 0.65` is the tightest crop that still clears 640px wide, and it is what puts his head on the others' scale; it also takes most of the notebook out, though a corner stays. **Two things no crop can fix and both are worth a look before launch** - he is the only one of the five not looking at the lens, and that notebook corner sits in the bottom of the card.

Worth noting for sign-off that likeness is not stable across passes: he wore glasses in his second frame and does not in his third or fourth.
...
**The masters are committed, which is a break from the other asset scripts.** `optimize-bg-video.mjs` reads its sources out of `~/Downloads`, and that is fine for a 100MB video that can be downloaded again. A generative output is one-shot and non-deterministic: clean that folder and it is gone for good. So the masters live in `docs/assets/team/`, named for the person rather than by whatever the tool that made them called the file - eight of them now, and the pipeline runs for anyone who clones the repo. The last three arrived as bare generative frames with no CMS record behind them, so their name-to-face mapping rests on the order they were sent rather than on anything in the files.
...
**Only the horizontal is cropped, except for Tatjana.** The four re-shoots are square, so top and bottom stay whole and `faceX` decides which side loses more of the trim. Lev and Lucas sit at a plain `0.5`: the centroid of the dark pixels in each master's hair band (y 15–32%) measures 0.485 and 0.539, which a centred crop puts within a couple of percent of the middle. **The other three all zoom, and every `zoom` in this file crops *in* - there is no entry that zooms out**, because the failure it fixes is always the same one, a master shot wider than the head-and-shoulders standard the set is built on. (An earlier revision of this note called Tatjana's "the opposite reason" to Bernardo's. That was wrong.)

Bernardo and Bruno are on their third and fourth masters, and between them they have carried five different sets of numbers. **Re-measure when a master changes rather than carrying its old number over**; every value in that list has been wrong at least once because someone did. Hers is a wider shot: her head fills about 32% of the frame against the others' ~40%, so at the same crop she read as standing further back than everyone else, which was the last thing breaking the row. **`zoom` exists for her alone.** 0.8 pulls her in to match, and it costs resolution rather than inventing any - the crop lands at 512×640 and `withoutEnlargement` leaves it there, about 6% under the card's 2× DPR ideal. A re-shoot of hers would close that properly; upscaling would only fake it.
...
**The bios are not rendered.** They came off on 7 September 2026 when the grid was rebuilt around the reference's photo-name-role card, went back the same day by request, and came off again on 9 September - "leave just the titles". A card is a portrait, a name and a job title. They are kept in `content/team.ts` rather than deleted, precisely because that sequence shows they can be wanted back; `app/about/page.tsx` carries the line to restore.
...
2. **The team bios are off the page as of 9 Sep 2026** - "leave just the titles" - so the drafted sentences about named individuals are no longer public. They are kept in `content/team.ts` and the constraint stands if they return: they describe the role, not the person, because names and titles are the only public facts and inventing career histories isn't a placeholder a reviewer can safely skim. **What replaces this item is smaller: confirm which of the two account executives is which.** They arrived as photographs plus a job title with no names; the names followed as a pair and are paired to the faces by send order alone. The section's *heading* is drafted too: mounting the thesis on `/about` put "Both Sides of The Table" on the page in full, and the team block had been borrowing that pillar as its heading and lede.
