...
2. **The team bios are off the page as of 9 Sep 2026** - "leave just the titles" - so the drafted sentences about named individuals are no longer public. They are kept in `content/team.ts` and the constraint stands if they return: they describe the role, not the person, because names and titles are the only public facts and inventing career histories isn't a placeholder a reviewer can safely skim. **What replaces this item is smaller: two of the eight have no name.** Two account executives arrived as photographs plus a job title, so both cards read "Name to come" - send the two names. The section's *heading* is drafted too: mounting the thesis on `/about` put "Both Sides of The Table" on the page in full, and the team block had been borrowing that pillar as its heading and lede.
