...
2. **The team bios describe the role, not the person** — deliberately. Names and titles are the only public facts; inventing career histories for five named individuals isn't a placeholder a reviewer can safely skim. Get two sentences from each of them. **They are back on the page as of 7 Sep 2026**, under each portrait, so this drafted copy is now public rather than sitting unrendered — get the two sentences from each of them before launch rather than after. The section's *heading* is drafted too: mounting the thesis on `/about` put "Both Sides of The Table" on the page in full, and the team block had been borrowing that pillar as its heading and lede.
